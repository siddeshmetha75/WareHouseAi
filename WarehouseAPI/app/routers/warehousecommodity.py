...
def create_warehouse_commodity(data: WarehouseCommodityCreate, db: Session = Depends(get_db), current_user: Users = Depends(require_auth_token)):
...
def get_warehouse_commodity(wc_id: int, db: Session = Depends(get_db), current_user: Users = Depends(require_auth_token)):
...
def update_warehouse_commodity(wc_id: int, data: WarehouseCommodityUpdate, db: Session = Depends(get_db), current_user: Users = Depends(require_auth_token)):
    wc = db.query(WarehouseCommodity).filter(WarehouseCommodity.Idwarehouse_commodity == wc_id).first()
    if not wc:
        raise HTTPException(status_code=404, detail="Warehouse Commodity not found")
...
@router.delete("/{wc_id}")
def soft_delete_warehouse_commodity(wc_id: int, db: Session = Depends(get_db), current_user: Users = Depends(require_auth_token)):
    wc = db.query(WarehouseCommodity).filter(WarehouseCommodity.Idwarehouse_commodity == wc_id).first()
    if not wc:
        raise HTTPException(status_code=404, detail="Warehouse Commodity not found")
...
def get_active_by_warehouse(warehouse_id: int, db: Session = Depends(get_db), current_user: Users = Depends(require_auth_token)):
    results = (
        db.query(
            WarehouseCommodity,
    ...
    response = []
    for wc, wname, cname, sname in results:
