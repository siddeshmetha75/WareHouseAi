...
def get_user_warehouses(user_id: int, db: Session = Depends(get_db)):
    ...
    """
    # Find all warehouse mappings for this user
    mappings = db.query(UserWarehouseMap).filter(UserWarehouseMap.User_id == user_id).all()
    if not mappings:
        raise HTTPException(status_code=404, detail="No warehouses found for this user")
